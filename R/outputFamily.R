...
outputFamily <- function(family, power=NULL){
  if(family=="binary"){
    link.inv <- sigmoid
    costfun <- devianceBernoulli
    gradfun <- function(Y, Y_hat) Y_hat - Y
  } else if(family=="multiclass"){
    link.inv <- softmax
    costfun <- devianceCategorical
    gradfun <- function(Y, Y_hat) Y_hat - Y
  } else if(family=="gaussian"){
    link.inv <- identity
    costfun <- devianceNormal
    gradfun <- function(Y, Y_hat) Y_hat - Y
  } else if(family=="poisson"){
    link.inv <- exp
    costfun <- deviancePoisson
    gradfun <- function(Y, Y_hat) Y_hat - Y
  } else if(family=="gamma"){
    link.inv <- exp
    costfun <- devianceGamma
    gradfun <- function(Y, Y_hat) 1 - Y/Y_hat
  } else if(family=="tweedie"){
    if(is.null(power)) stop("tweedie power parameter unspecified")
    link.inv <- exp
    costfun <- function(Y, Y_hat) devianceTweedie(Y, Y_hat, p=power)
